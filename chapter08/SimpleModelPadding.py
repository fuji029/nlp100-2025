...
class InputEmbedding:
    ...
    def __call__(self, input_ids: torch.Tensor, max_len: int):
        """
        入力埋め込みをBag of Wordsで作成
        """
        input_emb_tensor = torch.tensor(
            np.array([self.emb[id] for id in input_ids] + [self.emb[0] for _ in range(max_len - len(input_ids))]))
        input_emb = torch.mean(input_emb_tensor, 0, dtype=float)
        print(input_emb_tensor.shape)
        return input_emb
...
class MyDataLoader:
    ...
    def collate(self, dataset: list[dict]):
        x_batch = []
        y_batch = []
        batch = []
        ...
        for i, item in enumerate(dataset):
            ...
            if ((i+1) % self.batch_size == 0):
                ...
                x_batch = [embedding(input_ids, max_len)
                           for input_ids in x_batch]
                batch.append(
                    [torch.tensor(np.array(x_batch)), torch.tensor(np.array(y_batch))])
                x_batch = []
                y_batch = []
...
def main():
    ...
    train_batch = train_dataloader.collate(train_dataset)
    dev_batch = dev_dataloader.collate(dev_dataset)
